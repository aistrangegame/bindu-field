...
Last audited: **2026-05-22**, currently on `feat/stabilize` at `98fac48` (Session B's 8-tab structure landed: Map · Field · Oracle · AKASH · Archive · Lab · Ritual · Letter, plus the 33-chakra Map, the 7-step Consciousness Loop, the four-state Oracle redesign, custom Bindu tab glyphs, the Mini Player, audio exclusivity, and the 9-element Vocabulary renderer). Branch is up to date with `origin/feat/stabilize`, staged for device verification, not merged to main. Build clean, zero warnings, zero errors.
...
│       ├── Space/
│       │   ├── SpaceImmersedView.swift         # breath ring × beat modulation × rotating affirmation; takes an AudioSource param
...
- **`ChakraProtocol`** (`Models/ChakraProtocol.swift`) — the **dance protocols**. Nine chakras (root → maya) with `inhale/hold/exhale` (seconds), `beat`, `carrier`, `hue`, `essence`, and 5 `affirmations` each. Drives Space, Ritual, the Player's READING sheet (Breath row), and the Map detail sheet's metadata. Ordering for grid display lives in `SpaceSetupView` as `chakraOrder`.
...
| 3 | **AKASH** (tab label) / `SpaceView` (file) | `SpaceView` → `SpaceSetupView` ↔ `SpaceImmersedView` (`audioSource: .space`) | Pick chakra + duration (3/5/10/15 min, clamped to `defaultSessionDuration` initial). Immersed: breath ring expands on `inhale`, holds, contracts on `exhale` (seconds from `ChakraProtocol`). Beat modulation across phases (×1.0 / ×1.10 / ×0.80). Affirmation rotates every 20s. Claims the `.space` audio source on appear; releases on exit. Lock-screen Now Playing. Saves a `.chakra` `Session` on exit (skipped if < 5s). The tab label *says* AKASH; the file/store names still say Space. |
...
| 5 | **Lab** | `LabView` | Top: header dot + "frequency lab" italic + "craft your own permission slip" caption. **Animated binaural waveform** (`BinauralWaveformView`) — 120pt Canvas drawing L (cream 0.14) + R (state-hued 0.38) + bright beat envelope; ghost wave when inactive. Carrier + 76pt beat are **direct-editable** via `TextField(.decimalPad)` (clamps 40–440 / 0.5–44). State card (chevron-expandable) with the design's 5-band palette: delta=15°, theta=260°, alpha=210°, beta=165°, gamma=50°. Custom hairline sliders. **Sacred frequency strip** — 7 dots (136.1 OM, 174, 285, 396 UT, 417 RE, 432, 440) + glowing state-color carrier cursor. **`SacredBadge`** appears inline on the carrier row when ±1.2 Hz of a sacred carrier. **"let the field choose"** — weighted-state randomize (δ12/θ33/α37/β13/γ5%) + sacred-carrier ±0.5 jitter + 10-step animated cycling + spring lock. Preset row + inline save + long-press delete preserved. **Audio exclusivity**: claims `.lab` on ACTIVATE, releases on stop. Listens for `.binduLabStop`; another source's claim tears the tone down here. No Archive entry (by design). |
...
- Stores: `PlayerStore`, `SettingsStore`, `SessionStore`, `LetterStore`, `PresetStore`, `NavigationStore`, `TrackPlaybackService`, `DSPWireService`, `Performer`, `CatalogStore`, `AudioSessionCoordinator`, **`AudioExclusivityCoordinator`**, **`ChakraJourneyStore`**, **`ConsciousnessLoopCoordinator`**.
...
- **AKASH** (tag 3): breath-modulated binaural, beat ×1.0/1.1/0.8 across inhale/hold/exhale, affirmations rotate every 20s, lock-screen metadata, claims `.space` audio source, session saved if ≥5s.
- **Archive** (tag 4): grouped by date, Integration notes rendered, settings cog, clear archive + clear audio cache flows.
- **Lab** (tag 5): animated 3-layer binaural waveform + carrier and 76pt-beat readouts that are tap-to-edit via decimal pad + custom hairline sliders + sacred-frequency map strip (7 dots + glowing carrier cursor) + inline sacred badge (OM · C#3 etc. when ±1.2 Hz) + "let the field choose" weighted randomize + 5-band state palette + preset row with inline save and long-press delete. Claims `.lab` audio source on ACTIVATE; gives it up on a remote eviction.
...
- **Visual fidelity audit pending on device** for the non-Air vocabularies. The Cathedral is well-tested visually on Track 27; the other 8 vocabularies have shipped only against the design HTML. Sub-areas to inspect: verb-over-vocabulary legibility in FIELD across all 9 elements, Bindu vs. vocabulary-center collision at small device sizes, `vocab.bg` perceptual brightness across elements (some are slightly more saturated than others), atmospheric balance when `vizMode == "singular"` vs `"ensemble"`.
...
| `98fac48` | **docs: check in session handoffs, audits, and design HTMLs** *(current HEAD)* |

---

## 15. Where the design pass landed (and what's still ahead)

The Lalita pass shipped on `feat/lalita-pass` across six phases plus four post-review gap fixes. Session A landed three more design pieces (Oracle redesign, Tab Icons, Consciousness Loop). Session B landed the Map, the Mini Player, Element Vocabularies, and the AKASH rename + Field verbs. Stabilize landed audio exclusivity, readability, and dead-end fixes. The branch is staged for device verification, not merged.
...
None. Working tree is clean on `feat/stabilize`, up to date with `origin/feat/stabilize`. This CLAUDE.md update is the only working-tree modification at the time of this audit; commit it to capture the current state.
