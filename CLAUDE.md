...
| `da2921a` | **fix(lab): stepper precision matches readout precision** *(current HEAD)* — carrier fineStep 1.0 → 0.1; beat fineStep 0.1 → 0.01. Slider snap step unchanged at 0.1 (still the coarse control). User can now tap-step to 136.1 (OM) and 2.69 Hz without falling back to the keyboard. |
...
Only this CLAUDE.md edit. Working tree was clean on `feat/stabilize` at `da2921a`, five commits ahead of `origin/feat/stabilize`, not pushed. Commit this update to capture the stepper-precision tightening.
